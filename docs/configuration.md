...
Removing a runtime deletes its unneeded skills only when they match the saved raw baseline as well as the normalized install hash. Modified skills and legacy entries without a raw baseline are preserved with a warning. An update that leaves a base skill unchanged retains its previous raw baseline, even if an injection refresh changes the final bytes; a later removal may therefore preserve that skill conservatively.
